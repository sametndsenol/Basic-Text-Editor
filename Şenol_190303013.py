...
class Editor:
	...
	__thisWidth = 300
	__thisHeight = 300
	__thisTextArea=Text(Notebook,bg="white",font="Arial",fg="black",highlightbackground="red",highlightcolor="green",
	insertbackground="black",selectbackground="cyan",wrap=WORD)
	"""Theme changed to light, font black"""
	"""Create a text area, menu bar and scroll bar"""
	__thisMenuBar = Menu(Notebook)
	...
	def __init__(self,**kwargs):
		try:
			self.Notebook.wm_iconbitmap("Notepad.ico")
		except:
			pass
		try:
		...
		self.Notebook.title("Word - Editor")
...
#		self.__thisTextArea.bindtags(tuple(bindtags))
		...
		screenWidth = self.Notebook.winfo_screenwidth() 
		screenHeight = self.Notebook.winfo_screenheight() 
	
		left = (screenWidth / 2) - (self.__thisWidth / 2) 
		
		top = (screenHeight / 2) - (self.__thisHeight /2) 
		
		self.Notebook.geometry('%dx%d+%d+%d' % (self.__thisWidth,self.__thisHeight,left,top))
		...
		self.Notebook.config(menu = self.__thisMenuBar)
...
#		self.__thisTextArea.bind('<Control-j>',self.__SpellCheck)
	...
	def __openFile(self,event = None):
		...
		if self.__file == "":
			...
			self.Notebook.title(os.path.basename(self.__file)+" - Editor")
	...
	def __newFile(self, event = None):
		self.Notebook.title("Untitled - Editor")
		self.__file = None
		self.__thisTextArea.delete(1.0,END)	
	...
	def __find(self,event=None):
		...
		t1.join()
		"""
		t2=threading.Thread(target=self.__finder());
		t2.start()
		t2.join()
		"""
	...
	def __findNReplace(self,event=None):
		...
		t1.join()
		"""
		t2=threading.Thread(target=self.__findNReplaceF())
		t2.start()
		t2.join()
	"""
	"""
	#def __Checker(self,event = None):
...
Editor1 = Editor(width=800,height=600)
Editor1.run()
